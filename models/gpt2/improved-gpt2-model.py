...
class GPT2Model:
    ...
    async def process_request(self, data: bytes) -> None:
        ...
        try:
            ...
            if message_type == "text_generation":
                ...
                await self.send_error(f"Unknown message type: {message_type}", request.get("request_id"))
    ...
    async def handle_text_generation(self, request: Dict[str, Any]) -> None:
        ...
        if "prompt" not in request:
            await self.send_error("Missing 'prompt' field", request.get("request_id"))
            return
        ...
        try:
            ...
            await self.send_error(f"Error generating text: {str(e)}", request_id)
    ...
    async def send_error(self, error_message: str, request_id: Optional[str] = None) -> None:
        """
