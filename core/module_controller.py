...
# Import required classes for type annotations
...
class ModuleController:
    ...
    async def setup(self, interface: Cli_Chat, model: GPT2Model) -> bool:
        """
        Set up the controller with the provided interface and model.
        
        Args:
            interface: The interface module instance (e.g., Cli_Chat)
            model: The model module instance (e.g., GPT2Model)
            
        Returns:
            bool: True if setup was successful, False otherwise
        """
        # Store the interface and model instances
        self.interface = interface
        self.model = model
        
        # Create socket-pair streams
        ...
        self.interface_writer = interface_writer
        self.model_reader = model_reader
        self.model_writer = model_writer
        
        # Wire up the interface (similar to modcon_tmp.py)
        self.interface.interface_reader = interface_reader
        self.interface.interface_writer = interface_writer
        self.interface.model_reader = model_reader
        self.interface.model_writer = model_writer
        
        # Attach model to interface if it's Cli_Chat
        if hasattr(self.interface, '_gpt2'):
            self.interface._gpt2 = self.model
